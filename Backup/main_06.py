"""
Space invaders by Pedro Tortello - 24/01/2019
"""
...
def enemy_fire(enemy):
    global e_weaponstate
    e_weaponstate = 1
    # Move the missile to just below the enemy
    x = enemy.xcor()
    y = enemy.ycor() - 10
    e_weapon.goto(x, y)
...
def pause_game():
    ...
    if gameplay:
        ...
        for i in range(enemies_rows):
            for enemy in enemies[i]:
                enemy.hideturtle()
        disc_pen.write(f"Game paused", False, align="center",
...
screen.register_shape(".\Images\_destroy.gif")
...
screen.register_shape(".\Images\_e_laser.gif")
...
while not gameover:
    ...
    disc_bg = turtle.Turtle()
    disc_bg.speed(0)
    disc_bg.shape("square")
    disc_bg.shapesize(stretch_wid=2, stretch_len=22)
    disc_bg.hideturtle()
    disc_bg.color("black")
    disc_bg.penup()
    disc_bg.goto(0, 20)
    ...
    scorecor = []
    with open ('subs.ttl', "r") as f:
        for line in f:
            scorecor.append(line.strip())
        scorecor[0] = int(scorecor[0])
        scorecor[1] = int(subs.decrypt(scorecor[1]))
    if new_game:
        new_game = False
        scorecor[0] = 0
        with open ('subs.ttl', "w") as f:
            f.write(str(scorecor[0]) + "\n" + subs.encrypt(scorecor[1]))
    score_str = f"Score: {scorecor[0]}\t\t\tRecord: {scorecor[1]}"
    score_pen.write(score_str, False, align="center",
    ...
    e_weapon.speed(0)
    e_weapon.hideturtle()
    e_weapon.shape(".\Images\_e_laser.gif")
    e_weapon.penup()
    e_weapon.goto(0, 450)
    ...
    # Level loop
    disc_pen.write(f"Level {level}", False, align="center",
                    font=("Courier", 28, "normal"))
    time.sleep(3)
    disc_pen.clear()
    next_lvl = False
    while not next_lvl:
        ...
        for i in range(enemies_rows):
            for enemy in enemies[i]:
                ...
                if is_hit(weapon, enemy):
                    winsound.PlaySound(".\Sounds\s_explod.wav",
                                        winsound.SND_ASYNC)
                    weapon.hideturtle()
                    weaponstate = 0
                    weapon.goto(0, -450)
                    ...
                    if i < 1:
                        ...
                        scorecor[0] += 10
                    score_str = (
                        f"Score: {scorecor[0]}\t\t\tRecord: {scorecor[1]}"
                    )
                    score_pen.clear()
                    score_pen.write(score_str, False, align="center",
                                    font=("Courier", 12, "normal"))
                ...
                # Check for life loss
                if is_hit(player, enemy) or enemy.ycor() <= player.ycor() or (
                    is_hit(player, e_weapon)):
                    winsound.PlaySound(
                        ".\Sounds\s_life.wav", winsound.SND_ASYNC)
                    player.shape(".\Images\_destroy.gif")
                    e_weapon.hideturtle()
                    e_weaponstate = 0
                    e_weapon.goto(0, 450)
                    screen.update()
                    if lifes == 2:
                        ...
                        disc_bg.showturtle()
                        screen.update()
                        disc_pen.write(f"GAME OVER", False, align="center",
                                        font=("Courier", 28, "normal"))
                        time.sleep(3)
                        disc_pen.clear()
                        if scorecor[0] > scorecor[1]:
                            disc_pen.write(
                                f"New Record: {scorecor[0]}", False,
                                align="center", font=("Courier", 28, "normal")
                                )
                            time.sleep(3)
                            with open ('subs.ttl', "w") as f:
                                f.write(
                                    str(0) + "\n" + subs.encrypt(scorecor[0]))
                        else:
                            with open ('subs.ttl', "w") as f:
                                f.write(
                                    str(0) + "\n" + subs.encrypt(scorecor[1]))
        ...
        if e_weaponstate == 1:
            e_weapon.showturtle()
            e_weapon.sety(e_weapon.ycor() - e_weaponspeed)
        else:
            # Enemy firing
            row_to_col = list(zip(*enemies))
            armed = []
            for i in range(len(row_to_col)):
                lower = [400, 0]
                for enemy in row_to_col[i]:
                    if enemy.ycor() < lower[0]:
                        lower = [enemy.ycor(), enemy]
                if lower[0] < 400:
                    armed.append(lower[1])
            enemy_fire(random.choice(armed))
        if e_weapon.ycor() < -340:
            e_weapon.hideturtle()
            e_weaponstate = 0
            e_weapon.goto(0, 450)
        
        # Check for weapon vs weapon hit
        if is_hit(weapon, e_weapon):
            weapon.hideturtle()
            weaponstate = 0
            weapon.goto(0, -450)
            e_weapon.hideturtle()
